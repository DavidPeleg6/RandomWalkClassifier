# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 17:10:40 2021
...
def plot_confusion(confusion_mat, net_name, file_name):
    ...
    plt.title('rows=actual, columns=prediction {}'.format(net_name))
...
# net_type = ConvNet
location = 'variable length tests'
...
transition_prob = full_params['transition probability']
walkers = int(5e3)
...
if recreate_dataset:
    ...
    DatasetCreator.create_variable_length_dataset(diffusion=diffusion, delta_t=delta_t, steps=steps,
                                  ...
                                  names=names, train_name=train_name, test_name=test_name,
                                  max_transitions=2, transition_prob=transition_prob, param_name=label_name,
                                  alpha=alpha, min_steps=min_steps)
...
confusion, accuracy = create_confusion_matrix(train_loader, model, label_dict, percent=True)
